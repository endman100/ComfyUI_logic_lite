...
def get_comfyui_revision():
    try:
        ...
        print("Could not get ComfyUI revision, some features may not work as intended")
...
class imageSwitch:
    ...
    CATEGORY = "LogicLite/Logic/Switch"
...
class anythingIndexSwitch:
    ...
    CATEGORY = "LogicLite/Logic/Index Switch"
...
try:
    ...
    print("GraphBuilder not found, flow control nodes will not work")
    GraphBuilder = None
...
class whileLoopStart:
    ...
    CATEGORY = "LogicLite/Logic/While Loop"
    ...
    def explore_dependencies(self, node_id, dynprompt, upstream, parent_ids):
        ...
        for k, v in node_info["inputs"].items():
            if is_link(v):
                ...
                if class_type not in ['logic forLoopEnd', 'logic whileLoopEnd']:
...
class forLoopStart:
    ...
    CATEGORY = "LogicLite/Logic/For Loop"
    ...
    def for_loop_start(self, total, prompt=None, extra_pnginfo=None, unique_id=None, **kwargs):
        ...
        while_open = graph.node("logic whileLoopStart", condition=total, initial_value0=i, **initial_values)
...
class forLoopEnd:
    ...
    def for_loop_end(self, flow, dynprompt=None, extra_pnginfo=None, unique_id=None, **kwargs):
        ...
        if forstart_node['class_type'] == 'logic forLoopStart':
            inputs = forstart_node['inputs']
            total = inputs['total']
        elif forstart_node['class_type'] == 'logic loadImagesForLoop':
            inputs = forstart_node['inputs']
            limit = inputs['limit']
            start_index = inputs['start_index']
            # Filter files by extension
            directory = inputs['directory']
            total = graph.node('logic imagesCountInDirectory', directory=directory, limit=limit, start_index=start_index, extension='*').out(0)

        sub = graph.node("logic mathInt", operation="add", a=[while_open, 1], b=1)
        cond = graph.node("logic compare", a=sub.out(0), b=total, comparison='a < b')
        input_values = {("initial_value%d" % i): kwargs.get("initial_value%d" % i, None) for i in
                        range(1, MAX_FLOW_NUM)}
        while_close = graph.node("logic whileLoopEnd",
                                 flow=flow,
...
class Compare:
    ...
    def compare(self, a=0, b=0, comparison="a == b"):
        if comparison in ("a < b", "a > b", "a <= b", "a >= b"):
            if a is None or b is None:
                return (False,)
        if comparison in ("a > 0", "a <= 0") and a is None:
            return (False,)
        if comparison in ("b > 0", "b <= 0") and b is None:
            return (False,)
...
NODE_CLASS_MAPPINGS = {
    "logic string": String,
    "logic int": Int,
    "logic rangeInt": RangeInt,
    "logic float": Float,
    "logic rangeFloat": RangeFloat,
    "logic boolean": Boolean,
    "logic mathString": mathStringOperation,
    "logic mathInt": mathIntOperation,
    "logic mathFloat": mathFloatOperation,
    "logic compare": Compare,
    "logic imageSwitch": imageSwitch,
    "logic textSwitch": textSwitch,
    "logic imageIndexSwitch": imageIndexSwitch,
    "logic textIndexSwitch": textIndexSwitch,
    "logic conditioningIndexSwitch": conditioningIndexSwitch,
    "logic anythingIndexSwitch": anythingIndexSwitch,
    "logic ab": ab,
    "logic anythingInversedSwitch": anythingInversedSwitch,
    "logic whileLoopStart": whileLoopStart,
    "logic whileLoopEnd": whileLoopEnd,
    "logic forLoopStart": forLoopStart,
    "logic forLoopEnd": forLoopEnd,
    ...
    "logic pixels": pixels,
    "logic xyAny": xyAny,
    "logic lengthAnything": lengthAnything,
    "logic indexAnything": indexAnything,
    "logic batchAnything": batchAnything,
    "logic convertAnything": convertAnything,
    "logic showAnything": showAnything,
    "logic showTensorShape": showTensorShape,
    "logic clearCacheKey": clearCacheKey,
    "logic clearCacheAll": clearCacheAll,
...
NODE_DISPLAY_NAME_MAPPINGS = {
    "logic string": "String",
    "logic int": "Int",
    "logic rangeInt": "Range(Int)",
    "logic float": "Float",
    "logic rangeFloat": "Range(Float)",
    "logic boolean": "Boolean",
    "logic compare": "Compare",
    "logic mathString": "Math String",
    "logic mathInt": "Math Int",
    "logic mathFloat": "Math Float",
    "logic imageSwitch": "Image Switch",
    "logic textSwitch": "Text Switch",
    "logic imageIndexSwitch": "Image Index Switch",
    "logic textIndexSwitch": "Text Index Switch",
    "logic conditioningIndexSwitch": "Conditioning Index Switch",
    "logic anythingIndexSwitch": "Any Index Switch",
    "logic ab": "A or B",
    "logic anythingInversedSwitch": "Any Inversed Switch",
    "logic whileLoopStart": "While Loop Start",
    "logic whileLoopEnd": "While Loop End",
    "logic forLoopStart": "For Loop Start",
    "logic forLoopEnd": "For Loop End",
    "logic ifElse": "If else",
    "logic blocker": "Blocker",
    ...
    "logic pixels": "Pixels W/H Norm",
    "logic xyAny": "XY Any",
    "logic lengthAnything": "Length Any",
    "logic indexAnything": "Index Any",
    "logic batchAnything": "Batch Any",
    "logic convertAnything": "Convert Any",
    "logic showAnything": "Show Any",
    "logic showTensorShape": "Show Tensor Shape",
    "logic clearCacheKey": "Clear Cache Key",
    "logic clearCacheAll": "Clear Cache All",
